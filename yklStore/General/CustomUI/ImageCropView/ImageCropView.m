...
//                                                  action:@selector(done:)];
        ...
        CGRect view = CGRectMake(0, 64, self.view.bounds.size.width, self.view.bounds.size.height - [[self navigationController] navigationBar].bounds.size.height - statusBarSize.height);
        
        self.cropView  = [[ImageCropView alloc] initWithFrame:view blurOn:self.blurredBackground type:_type];
...
- (id)initWithFrame:(CGRect)frame blurOn:(BOOL)blurOn type:(int)type
{
    self = [super initWithFrame:frame];
    if (self) {
        self.blurred = blurOn;
...
- (void)boundingBoxForTopLeft:(CGPoint)topLeft bottomLeft:(CGPoint)bottomLeft bottomRight:(CGPoint)bottomRight topRight:(CGPoint)topRight {
    ...
    switch (_type) {
        ...
        case 2:
            ...
            box.size.width = box.size.height/2*3;
    ...
    if (box.origin.x + box.size.width - controlPointSize > imageView.frame.size.width) {
