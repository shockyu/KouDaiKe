//
//  YKLPrizesActivityListViewController.m
//  yklStore
//
...
const float YKLPrizesActivityListUpViewH = 40;
@interface YKLPrizesActivityListViewController ()<YKLPrizesActivityListViewDelegate>
...
@implementation YKLPrizesActivityListViewController
...
- (float)rainshedLaceOffset {
    return YKLPrizesActivityListUpViewH;
}

- (void)show {
    self.upView.top = 0;
    self.scrollView.top = YKLPrizesActivityListUpViewH;
}
...
- (void)viewDidLoad {
    ...
    self.upView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, self.contentView.width, YKLPrizesActivityListUpViewH)];
    self.upView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:self.upView];
    
    self.typeSegment.frame = CGRectMake(0, 0, self.contentView.width, YKLPrizesActivityListUpViewH);
    [self.upView addSubview:self.typeSegment];
    
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, YKLPrizesActivityListUpViewH+64, self.contentView.width, self.contentView.height-YKLPrizesActivityListUpViewH)];
...
- (void)refreshOrderList {
    ...
    YKLPrizesActivityListView *payingList = [self.orderListDictionary objectForKey:@0];
    [payingList refreshList];
    YKLPrizesActivityListView *waitList = [self.orderListDictionary objectForKey:@1];
    [waitList refreshList];
    YKLPrizesActivityListView *doneList = [self.orderListDictionary objectForKey:@2];
    [doneList refreshList];
}

- (void)typeSegmentValueChanged:(UISegmentedControl *)segment {
    
    YKLPrizesActivityListView *listView = [self.orderListDictionary objectForKey:[NSNumber numberWithInteger:segment.selectedSegmentIndex]];
    ...
    if (listView == nil) {
        ...
        listView = [[YKLPrizesActivityListView alloc] initWithFrame:CGRectMake(segment.selectedSegmentIndex*self.scrollView.width, 0, self.scrollView.width, self.scrollView.height) orderType:type];
...
- (void)showYKLActivityListTypeWaitDetailView:(YKLPrizesActivityListView *)listView didSelectOrder:(YKLRedActivityListModel *)model {
...
- (void)showActivityListTypeIngDetailView:(YKLPrizesActivityListView *)listView didSelectOrder:(YKLRedActivityListModel *)model {
...
- (void)showYKLActivityListTypeEndDetailView:(YKLPrizesActivityListView *)listView didSelectOrder:(YKLRedActivityListModel *)model {
