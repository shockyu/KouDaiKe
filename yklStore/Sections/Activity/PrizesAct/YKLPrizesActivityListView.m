...
@implementation YKLPrizesActivityListView
...
- (instancetype)initWithFrame:(CGRect)frame orderType:(YKLActivityListType)type {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.tableView registerClass:[YKLPrizesActivityListTableViewCell class] forCellReuseIdentifier:@"Cell"];
...
- (void)getActivitylist:(NSString *)status{
    ...
    [YKLNetworkingPrizes getRedListWithUserID:[YKLLocalUserDefInfo defModel].userID Status:status Success:^(NSArray *activityList) {
...
//     让表格缓冲区查找可重用cell
    YKLPrizesActivityListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
//     如果没有找到可重用cell
    if (cell == nil) {
        // 实例化cell
        cell = [[YKLPrizesActivityListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    ...
    if (self.type == YKLActivityListTypeIng) {
        ...
        cell.againReleaseBtn.hidden = YES;
        
        if (self.activityList.count){
            self.model = self.activityList[indexPath.row];
            cell.titleLabel.text = self.model.title;
            
            cell.originalPriceLabel.text = [NSString stringWithFormat:@"%@人",self.model.prizesCount];
            cell.bargainPriceLabel.text = [NSString stringWithFormat:@"%@份",self.model.flowCount];
            
            cell.participantNubLabel.text = [NSString stringWithFormat:@"%@个",self.model.surplusPrizesNum];
            cell.successNubLabel.text = [NSString stringWithFormat:@"%@份",self.model.surplusFlowNum];
...
//            self.model = self.activityList[indexPath.row];
            ...
            [cell.againReleaseBtn addTarget:self action:@selector(againReleaseBtnClick:) forControlEvents:UIControlEventTouchUpInside];
            cell.againReleaseBtn.tag = indexPath.row;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (self.type) {
        case 0:
            [self.delegate showPrizesActivityListTypeIngDetailView:self didSelectOrder:self.activityList[indexPath.row]];
            break;
        case 1:
            [self.delegate showYKLPrizesActivityListTypeWaitDetailView:self didSelectOrder:self.activityList[indexPath.row]];
            break;
        case 2:
            [self.delegate showYKLPrizesActivityListTypeEndDetailView:self didSelectOrder:arry[row]];
            break;
...
- (void)againReleaseBtnClick:(UIButton *)sender{
    ...
    [self.delegate againReleaseSelectPrizesOrder:self.model.activityID];
}
...
- (void)shareBtnClick:(UIButton *)sender{
    ...
    //调取显示分享页面方法
    [self.delegate showShareViewDidSelectPrizesOrder:self.model isPay:@"NO"];
}
...
- (void)shareReleaseBtnClick:(UIButton *)sender{
    ...
    if ([[YKLLocalUserDefInfo defModel].userID isEqual:@"29"]) {
        ...
        [YKLNetworkingPrizes releasePublicRedWithRID:self.model.activityID Success:^(NSDictionary *dict) {
            ...
            if (![[dict objectForKey:@"pay"] isEqual:@""]) {
                ...
                [self.delegate payViewPrizeDidSelectOrder:dict ActID:self.model.activityID];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        [YKLNetworkingPrizes deleteRedWithRedID:self.deletActivityID Success:^(NSDictionary *dict) {
            //刷新待发布活动列表
            [self getActivitylist:@"2"];
        } failure:^(NSError *error) {
            [UIAlertView showInfoMsg:error.domain];
