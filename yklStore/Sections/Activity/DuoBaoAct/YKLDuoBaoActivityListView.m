...
@implementation YKLDuoBaoActivityListView
...
- (instancetype)initWithFrame:(CGRect)frame orderType:(YKLActivityListType)type {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.tableView registerClass:[YKLDuoBaoActivityListTableViewCell class] forCellReuseIdentifier:@"Cell"];
...
- (void)getActivitylist:(NSString *)status{
    ...
    [YKLNetworkingDuoBao getIndianaListWithUserID:[YKLLocalUserDefInfo defModel].userID Status:status Success:^(NSArray *activityList) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //     让表格缓冲区查找可重用cell
    YKLDuoBaoActivityListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    //     如果没有找到可重用cell
    if (cell == nil) {
        // 实例化cell
        cell = [[YKLDuoBaoActivityListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (self.type) {
        case 0:
            [self.delegate showDuoBaoActivityListTypeIngDetailView:self didSelectOrder:self.activityList[indexPath.row]];
            break;
        case 1:
            [self.delegate showYKLDuoBaoActivityListTypeWaitDetailView:self didSelectOrder:self.activityList[indexPath.row]];
            break;
        case 2:
            [self.delegate showYKLDuoBaoActivityListTypeEndDetailView:self didSelectOrder:arry[row]];
            break;
...
- (void)shareBtnClick:(UIButton *)sender{
    ...
    //调取显示分享页面方法
    [self.delegate showShareViewDidSelectDuoBaoOrder:self.model isPay:@"NO"];
}
...
//    }
    ...
    if ([[YKLLocalUserDefInfo defModel].userID isEqual:@"29"]) {
        ...
        [YKLNetworkingDuoBao releasePublicIndianaWithIndianaID:self.model.activityID Success:^(NSDictionary *dict) {
            ...
            if ([[dict objectForKey:@"pay"] isEqual:@""] || [[dict objectForKey:@"pay"] isEqual:@[]]) {
                ...
                [self.delegate payViewDuoBaoDidSelectOrder:dict ActID:self.model.activityID];
            }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        [YKLNetworkingDuoBao deleteIndianaWithIndianaID:self.deletActivityID Success:^(NSDictionary *dict) {
            //刷新待发布活动列表
            [self getActivitylist:@"2"];
        } failure:^(NSError *error) {
            [UIAlertView showInfoMsg:error.domain];
