...
@implementation YKLMiaoShaActivityListView
...
- (instancetype)initWithFrame:(CGRect)frame orderType:(YKLActivityListType)type {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.tableView registerClass:[YKLMiaoShaActivityListTableViewCell class] forCellReuseIdentifier:@"Cell"];
        ...
        //注册通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isSelected:) name:@"YYMSQuanXuanIsSelected"object:nil];
    }
    return self;
}
...
- (void)getActivitylist:(NSString *)status{
    ...
    [YKLNetworkingMiaoSha getActListWithUserID:[YKLLocalUserDefInfo defModel].userID Status:status Success:^(NSArray *activityList) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //     让表格缓冲区查找可重用cell
    YKLMiaoShaActivityListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    //     如果没有找到可重用cell
    if (cell == nil) {
        // 实例化cell
        cell = [[YKLMiaoShaActivityListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    ...
    if (self.type == YKLActivityListTypeIng) {
        ...
        if (self.activityList.count){
            ...
            cell.startTimeLabel.text = [NSString stringWithFormat:@"开始时间：%@",self.model.startTime];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (self.type) {
        case 0:
            [self.delegate showMiaoShaActivityListTypeIngDetailView:self didSelectOrder:self.activityList[indexPath.row]];
            break;
        case 1:
            [self.delegate showYKLMiaoShaActivityListTypeWaitDetailView:self didSelectOrder:self.activityList[indexPath.row]];
            break;
        case 2:
            [self.delegate showYKLMiaoShaActivityListTypeEndDetailView:self didSelectOrder:arry[row]];
            break;
...
- (void)showShareButtonClick{
    ...
    if (deleteArr.count == 0) {
        ...
        [YKLNetworkingMiaoSha shareActWithActArray:str Success:^(NSDictionary *shareDic) {
            ...
            [self.delegate showShareViewDidSelectMiaoShaOrder:self.model isPay:@"NO"];
...
- (void)cancelButtonClick{
    ...
    [self.delegate cancer_YYMS_HB_ShareClicked];
...
- (void)shareReleaseBtnClick:(UIButton *)sender{
    ...
    if ([[YKLLocalUserDefInfo defModel].userID isEqual:@"29"]) {
        ...
        [YKLNetworkingMiaoSha releaseActWithActID:self.model.activityID Success:^(NSDictionary *dict) {
...
//                [UIAlertView showInfoMsg:@"活动开始日期已过，请重新发布！"];
            ...
            if ([[dict objectForKey:@"pay"] isEqual:@""] || [[dict objectForKey:@"pay"] isEqual:@[]]) {
                ...
                [self.delegate payViewMiaoShaDidSelectOrder:dict ActID:self.model.activityID];
            }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        [YKLNetworkingMiaoSha delActWithActID:self.deletActivityID Success:^(NSDictionary *dict) {
            //刷新待发布活动列表
            [self getActivitylist:@"2"];
        } failure:^(NSError *error) {
            [UIAlertView showInfoMsg:error.domain];
