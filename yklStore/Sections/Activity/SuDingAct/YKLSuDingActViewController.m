...
const float YKLSuDingActivityListUpViewH = 40;
@interface YKLSuDingActViewController ()<YKLSuDingActivityListViewDelegate>
...
@implementation YKLSuDingActViewController
...
- (UISegmentedControl *)typeSegment {
    if (_typeSegment == nil) {
        ...
        if ([[YKLLocalUserDefInfo defModel].actType isEqualToString:@"进行中"]) {
            ...
            [self.menuDelegate changeRightItemHiden:YES type:@"速定"];
        }
        else if([[YKLLocalUserDefInfo defModel].actType isEqualToString:@"待发布"]){
            _typeSegment.selectedSegmentIndex = 1;
            [self.menuDelegate changeRightItemHiden:NO type:@"速定"];
        }
...
- (void)viewDidLoad {
    ...
    self.upView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, self.contentView.width, YKLSuDingActivityListUpViewH)];
    self.upView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:self.upView];
    
    self.typeSegment.frame = CGRectMake(0, 0, self.contentView.width, YKLSuDingActivityListUpViewH);
    [self.upView addSubview:self.typeSegment];
    
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, YKLSuDingActivityListUpViewH+64, self.contentView.width, self.contentView.height-YKLSuDingActivityListUpViewH)];
...
- (void)refreshOrderList {
    ...
    YKLSuDingActivityListView *payingList = [self.orderListDictionary objectForKey:@0];
    [payingList refreshList];
    YKLSuDingActivityListView *waitList = [self.orderListDictionary objectForKey:@1];
    [waitList refreshList];
    YKLSuDingActivityListView *doneList = [self.orderListDictionary objectForKey:@2];
                           ...
                           Show:(BOOL)showBool
{
    ...
    YKLSuDingActivityListView *listView = [self.orderListDictionary objectForKey:[NSNumber numberWithInteger:segment.selectedSegmentIndex]];
    ...
    if (listView == nil) {
        ...
        listView = [[YKLSuDingActivityListView alloc] initWithFrame:CGRectMake(segment.selectedSegmentIndex*self.scrollView.width, 0, self.scrollView.width, self.scrollView.height) orderType:type];
...
- (void)showYKLActivityListTypeWaitDetailView:(YKLSuDingActivityListView *)listView didSelectOrder:(YKLMiaoShaActivityListModel *)model {
...
- (void)showActivityListTypeIngDetailView:(YKLSuDingActivityListView *)listView didSelectOrder:(YKLMiaoShaActivityListModel *)model {
...
- (void)showYKLActivityListTypeEndDetailView:(YKLSuDingActivityListView *)listView didSelectOrder:(YKLMiaoShaActivityListModel *)model {
