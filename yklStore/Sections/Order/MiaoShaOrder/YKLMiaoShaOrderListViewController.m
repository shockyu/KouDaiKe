//
//  YKLMiaoShaOrderListViewController.m
//  yklStore
//
...
#import "YKLMiaoShaOrderListView.h"

@interface YKLMiaoShaOrderListViewController ()<YKLMiaoShaOrderListViewDelegate>
...
@implementation YKLMiaoShaOrderListViewController
...
- (void)typeSegmentValueChanged:(UISegmentedControl *)segment {
    ...
    YKLMiaoShaOrderListView *listView = [self.orderListDictionary objectForKey:[NSNumber numberWithInteger:segment.selectedSegmentIndex*2]];
    ...
    if (listView == nil) {
        ...
        listView = [[YKLMiaoShaOrderListView alloc] initWithFrame:CGRectMake(segment.selectedSegmentIndex*self.scrollView.width*2, 0, self.scrollView.width, self.scrollView.height) orderType:type orderID:self.orderID];
...
- (void)typeSegmentValueFaceChanged:(UISegmentedControl *)segment {
    ...
    YKLMiaoShaOrderListView *listView = [self.orderListDictionary objectForKey:[NSNumber numberWithInteger:segment.selectedSegmentIndex]];
    ...
    if (listView == nil) {
        ...
        listView = [[YKLMiaoShaOrderListView alloc] initWithFrame:CGRectMake(segment.selectedSegmentIndex*self.scrollView.width, 0, self.scrollView.width, self.scrollView.height) orderType:type orderID:self.orderID];
        listView.delegate = self;
        [self.scrollView addSubview:listView];
