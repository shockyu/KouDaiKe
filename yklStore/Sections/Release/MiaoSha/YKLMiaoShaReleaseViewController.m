//
//  YKLMiaoShaReleaseViewController.m
//  yklStore
//
...
@interface YKLMiaoShaReleaseViewController ()<UITextFieldDelegate,UITextViewDelegate,UIPickerViewDataSource, UIPickerViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate,ImageCropViewControllerDelegate>
...
@implementation YKLMiaoShaReleaseViewController
...
- (void)viewDidLoad {
    ...
    if (!(self.activityID == NULL)) {
        ...
        [YKLNetworkingMiaoSha getActListWithActID:self.activityID Success:^(NSDictionary *dict) {
            ...
            self.payNunField.text = [tempDict objectForKey:@"once_max"];
            ...
            [self.showStartTimebtn setTitle:[NSString timeNumberHHmm:[tempDict objectForKey:@"start_time"]] forState:UIControlStateNormal];
            ...
            if ([continueTime isEqualToString:@"10"]) {
                [self Min10BtnClicked];
            }
            else if ([continueTime isEqualToString:@"20"]) {
                [self Min20BtnClicked];
            }
            else if ([continueTime isEqualToString:@"30"]) {
    ...
    //如果本地存储活动字典不为空则加载其数据
    else if (![[YKLLocalUserDefInfo defModel].saveMiaoShaActInfoDict isEqual:@{}]) {
        
        NSMutableDictionary *tempDict = [YKLLocalUserDefInfo defModel].saveMiaoShaActInfoDict;
        ...
        [self.showStartTimebtn setTitle:[tempDict objectForKey:@"start_time"] forState:UIControlStateNormal];
        [self.actTemplateChangeBtn setTitle:[tempDict objectForKey:@"template_name"] forState:UIControlStateNormal];
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    //首次进入弹出帮助页面
    if ([[YKLLocalUserDefInfo defModel].miaoShaHelp isEqual:@"YES"]) {
        
        [self showHelpView];
        
        [YKLLocalUserDefInfo defModel].miaoShaHelp = @"NO";
...
- (void)createBgView{
    ...
    //全部显示视图可移动大小
    self.scrollView.contentSize = CGSizeMake(self.view.width, 1132);
    ...
    self.bgView2 = [[UIView alloc]initWithFrame:CGRectMake(0, self.bgView1.bottom+15, self.view.width, 190)];
...
- (void)createContentBgView1{
    ...
    self.miaoShaField.placeholder = @"请设置秒杀价";
...
- (void)createContentBgView2{
    ...
    [self.showStartTimebtn setTitle:@"请点击设置秒杀开始时间" forState:UIControlStateNormal];
    ...
    titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, lineView.bottom, 70, 30)];
    titleLabel.font = [UIFont systemFontOfSize:14];
    titleLabel.text = @"秒杀时间:";
    [self.bgView2 addSubview:titleLabel];
    
    self.Min10Btn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.Min10Btn.frame = CGRectMake(titleLabel.right+10, 0, 15, 15);\
    ...
    titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.Min10Btn.right, lineView.bottom, 50, 30)];
    titleLabel.font = [UIFont systemFontOfSize:14];
    titleLabel.text = @"10分钟";
    [self.bgView2 addSubview:titleLabel];
    
    self.Min20Btn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.Min20Btn.frame = CGRectMake(titleLabel.right+10, 0, 15, 15);\
...
//    self.Min20Btn.selected = YES;
    ...
    titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.Min20Btn.right, lineView.bottom, 50, 30)];
    titleLabel.font = [UIFont systemFontOfSize:14];
    titleLabel.text = @"20分钟";
    [self.bgView2 addSubview:titleLabel];
    
    self.Min30Btn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.Min30Btn.frame = CGRectMake(titleLabel.right+10, 0, 15, 15);\
    ...
    titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.Min30Btn.right, lineView.bottom, 50, 30)];
    titleLabel.font = [UIFont systemFontOfSize:14];
    titleLabel.text = @"30分钟";
    [self.bgView2 addSubview:titleLabel];
...
- (void)createContentBgView3{
    ...
    self.ruleDesc.text = @"1.看中产品后，可报名参加活动，待秒杀活动开始后才可进行秒杀；\n2.在秒杀活动开始前十分钟系统将发布短信通知您即将开始秒杀；\n3.秒杀活动开放时间一般为半个小时，产品有限，时间有限，先到先得；\n4.秒杀成功的客户，需凭秒杀成功的手机页面，到门店兑换；\n5.活动页面产品图片及信息仅供参照，产品以实物为准；\n6.兑奖只以活动兑奖页面显示的内容为准，截图或短信均为无效。\n7.本活动最终解释权归本店所有。";
...
- (void)actTemplateChangeBtnClickClick:(id)sender{
    ...
    YKLMiaoShaReleaseTemplateViewController *releaseVC = [YKLMiaoShaReleaseTemplateViewController new];
...
- (void)saveGoodsBtnClick:(UIButton *)sender{
    ...
    if ([self.payNunField.text isBlankString]) {
        [UIAlertView showInfoMsg:@"请设置单个用户购买上限"];
        [self.payNunField becomeFirstResponder];
        return;
    }
    ...
    if([self.showStartTimebtn.titleLabel.text isEqualToString:@"请点击设置秒杀开始时间"]){
        [UIAlertView showInfoMsg:@"请点击设置秒杀开始时间"];
        return;
    }
    ...
    if (miaoShaPrice > price || miaoShaPrice == price) {
        ...
        [UIAlertView showInfoMsg:@"秒杀价不能大于等于原价"];
        [self.priceField becomeFirstResponder];
        return;
    }
...
- (void)releaseAct{
    ...
    [YKLNetworkingMiaoSha releaseActWithData:str Success:^(NSDictionary *dict) {
        ...
        if ([dict isEqual:@"活动开始日期已过，请重新发布！"]) {
            [UIAlertView showInfoMsg:@"活动开始日期已过，请重新发布！"];
...
- (void)saveDictForFiled{
    ...
    [parameters setObject:self.payNunField.text forKey:@"once_max"];
    [parameters setObject:rewardCode forKey:@"reward_code"];
    [parameters setObject:self.ruleDesc.text forKey:@"rule_desc"];
    
    [parameters setObject:self.showStartTimebtn.titleLabel.text forKey:@"start_time"];
    ...
    [YKLLocalUserDefInfo defModel].saveMiaoShaActInfoDict = parameters;
    [[YKLLocalUserDefInfo defModel] saveToLocalFile];
