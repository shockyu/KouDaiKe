//
//  YKLReleaseTemplateViewController.m
//  yklStore
//
...
@implementation YKLReleaseTemplateView
...
- (void)getActivitylist:(NSString *)type{
    ...
    [YKLNetworkingConsumer getTemplateWithType:type
                                      TempCode:self.tempCode
...
@interface YKLReleaseTemplateViewController ()<YKLReleaseTemplateViewDelegate>
{
...
@implementation YKLReleaseTemplateViewController
...
- (void)typeSegmentValueFaceChanged:(NSInteger)segment TempCode:(NSString *)tempCode {
    ...
    YKLReleaseTemplateView *releaseTemplateView = [self.orderListDictionary objectForKey:[NSNumber numberWithInteger:segment]];
    ...
    if (segment == 0) {
        ...
        releaseTemplateView = [[YKLReleaseTemplateView alloc] initWithFrame:CGRectMake(segment*self.scrollView.width, 0, self.scrollView.width, self.scrollView.height) TempCode:@""];
    }
    else if (segment == 1) {
        
        releaseTemplateView = [[YKLReleaseTemplateView alloc] initWithFrame:CGRectMake(segment*self.scrollView.width, 0, self.scrollView.width, self.scrollView.height) TempCode:tempCode];
    }
...
- (void)consumerTemplateListView:(YKLReleaseTemplateView *)listView didSelectOrder:(YKLTemplateModel *)model{
    ...
    YKLReleaseViewController *releaseVC = [self.navigationController.viewControllers objectAtIndex:self.navigationController.viewControllers.count-2];
    //初始化其属性
    releaseVC.actExpTextLabel.text = nil;
    //传递参数过去
    releaseVC.actExpTextLabel.text = model.templateName;
...
- (void)cateBtnClicked:(UIButton *)aBtn
{
    ...
    YKLReleaseTemplateView *templateView = (YKLReleaseTemplateView *)[self.scrollView viewWithTag:kTemplateViewTag];
