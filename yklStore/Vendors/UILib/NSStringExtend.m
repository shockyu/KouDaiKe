...
+ (BOOL)isNumberString:(NSString *)str
{
    ...
    NSRegularExpression *regularexpression = [[NSRegularExpression alloc]
            initWithPattern:@"^[0-9]*$"/*@"^[\\+-]?(?:|0|[1-9]\\d{0,})(?:\\.\\d*)?$"*/
...
{
    return [NSString isFloatNumberString:self];
}
...
- (BOOL)isHttpString
{
    return [NSString isHttpString:self];
}
...
+ (BOOL)isTelephoneNumber:(NSString *)str
{
    ...
    if ([regextesttel evaluateWithObject:str] == YES)
        return YES;
...
{
    return [NSString isTelephoneNumber:self];
}
...
- (UInt32)hexToUInt
{
    return [NSString hexToUInt:self];
}

- (NSDate *)toNSDate:(NSString *)szFormat
{
    return [NSDate formatNSStringDate:self format:szFormat];
}

- (NSString *)timeNumber{
    return [NSString timeNumber:self];
}
...
- (NSDecimalNumber *)getNSRoundPlain:(int)num
{
    ...
    NSDecimalNumber *subtotal = [NSDecimalNumber decimalNumberWithString:@"0.0"];
    
    NSDecimalNumber *discount = [NSDecimalNumber decimalNumberWithString:self];
...
- (NSString *)isBankNumber{
    return [NSString returnBankName:self];
}
...
//    if(idCard==nil || idCard.length<17 || idCard.length>20){
//        return @"不支持的银行卡类型";
//    }
