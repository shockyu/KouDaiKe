...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
        ...
        // add by will
        self.backgroundColor = BZViewBGColor;

        UIFont *ft = [UIFont boldSystemFontOfSize:12.f];
        _stateLabel = [[UILabel alloc] init ];
        _stateLabel.font = ft;
        _stateLabel.textColor = [UIColor blackColor];//kTitleLabelColor;
        ...
        _dateLabel.font = ft;
        _dateLabel.textColor = [UIColor blackColor];//kTitleLabelColor;
...
//    x = 0;
    ...
    if (self.isAtTop) {
        ...
        _arrow.contents = (id)arrow.CGImage;
        _stateLabel.text = NSLocalizedString(@"上拉载入", @"");
    }
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            if (self.isAtTop) {
                ...
                _stateLabel.text = NSLocalizedString(@"释放载入更多", @"");
            }
            ...
            if (self.isAtTop) {
                ...
                _stateLabel.text = NSLocalizedString(@"上拉载入更多", @"");
            }
...
- (void)setHideHeader:(BOOL)hideHeader{
    ...
    _headerView.hidden = _hideHeader;
}
...
//    CGSize contentSize = scrollView.contentSize;
    ...
    {
        // add by will         // 这里判断如果是 无header 的话, 就直接返回
        if (self.hideHeader) {
            return;
        }
